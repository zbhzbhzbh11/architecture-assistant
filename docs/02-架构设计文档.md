...
| G5 | 工程可维护 | 7 微服务独立部署，代码量可控，通过环境变量驱动配置 |
...
**设计决策**:
- 7 个微服务各司其职，单文件最大 393 行，平均约 200 行
...
graph TB
    User["👤 用户<br/>(课程学生/开发者)"]
    System["🏛 架构风格智能助手<br/>Architecture Assistant<br/>7 微服务 + Neo4j + 前端"]
...
候选列表 + 需求文本 + 特征
  ...
  │
  ├─ 4. 构建对比矩阵 (3 行 × 7 列)
  │    列: recommendation_type, style, score, key_reasons, pros, cons, topology_mermaid
  │
...
`POST /feedback` 接收用户对推荐结果的确认或修正，存储到 `feedback_log.json`。

### 17.2 自动权重学习

`json_repository.py:_update_learned_weights()` (第 170-196 行):

```
用户确认推荐 (is_confirmed=true)
  │
  ├─ 1. _extract_features_from_requirement()
  │    通过 FEEDBACK_LEXICON 词典从需求文本提取特征
  │
  ├─ 2. 对提取到的特征 → 推荐风格的关联权重 +1
  │    存储结构: learned_weights[feature][style] = count
  │
  └─ 3. 持久化到 learned_weights.json
```

### 17.3 权重应用

matching-agent 评分时读取学习权重：`count >= 2` 的特征-风格关联为该风格 +1 分。阈值设计防止单次反馈过度影响推荐。
...
```json
{
  ...
  "accuracy": 0.75,
  "style_stats": {
    "Layered Architecture": {"received": 3, "confirmed": 3},
    "Client-Server": {"received": 1, "confirmed": 0}
  }
}
```
...
| 对比矩阵 | ✅ 7 列 × 3 行 | `evaluation-agent/main.py` |
...
| # | 技术建议项 | 实现情况 | 实现方式 |
|---|-----------|---------|---------|
| 1 | 微服务架构 + Agent 协作 | ✅ | 7 微服务 + LangGraph/手动编排 |
