...
| `match_node` | matching-agent:8002/match | **LangGraph Subgraph** (3 子节点: rule_score → graph_blend → combo_rank) | extracted_features | candidates, combination_candidates |
...
graph LR
    ...
    subgraph match_subgraph["matching-agent Subgraph (3 子节点)"]
        direction LR
        MS((START)) --> rule_score
        rule_score --> graph_blend
        graph_blend --> combo_rank
        combo_rank --> ME((END))
    end
...
| `graph_blend` | HTTP 拉取图谱证据, blend_scores() 融合 + select_top3() | `common.matching.blend_scores`, `select_top3` |
| `combo_rank` | HTTP 拉取组合定义, rank_combinations() 排序 | `common.matching.rank_combinations` |
...
需求文本
  ...
  │    Prompt: 10-shot Few-shot → 零样本 fallback
  │    Temperature: 0.3, Timeout: 15s
  │    输出: 全 10 维 True/False JSON + 架构倾向 (---ARCH---)
...
**10 维特征词典** (外置于 `feature_lexicon.json`):

| 维度 key | 中文名 | 关键词数 | 说明 |
|----------|--------|----------|------|
| high_concurrency | 高并发 | 15 | 高并发/万人/峰值/高负载/大流量/亿级 |
| real_time | 实时性 | 15 | 实时/毫秒/推送/同步/streaming |
| reliability | 可靠性 | 16 | 高可用/容灾/sla/熔断/failover |
| scalability | 可扩展性 | 14 | 弹性/水平扩展/集群/分片/sharding |
| complex_business | 复杂业务 | 16 | 工作流/审批/CMS/ERP/多模块 |
| strict_consistency | 强一致性 | 15 | 事务/ACID/原子性/分布式事务/回滚 |
| deployment_constraint | 部署约束 | 14 | 本地部署/内网/信创/国产化/局域网 |
| data_intensive | 数据密集型 | 18 | 数据流/ETL/大数据/数据仓库/BI/TB级 |
| team_size_large | 多团队 | 12 | 多团队/跨团队/多部门/独立交付 |
| security | 安全性 | 19 | 加密/审计/等保/零信任/GDPR |
...
**文件**: `services/matching_agent/app/main.py` (95 行) + `matching_subgraph.py` (180 行) + `graph_matcher.py` (67 行) + `combo_matcher.py` (33 行)

**共享模块**: `services/common/matching/` — `rules.py` (score_style, select_top3), `graph.py` (blend_scores), `combo.py` (score_combination, rank_combinations)

**设计思路**: 三层评分体系——规则引擎（确定性）+ 知识图谱（关系推理）+ LLM 投票（语义增强）。匹配流程由 3 子节点 LangGraph Subgraph 驱动: rule_score → graph_blend → combo_rank。
...
| `graph_blend` | HTTP 拉取图谱证据, blend_scores() 融合 + select_top3() Top3 选择 | `candidates` |
| `combo_rank` | HTTP 拉取 5 种组合定义, rank_combinations() 评分排序 | `combination_candidates` |
...
**文件**: `services/evaluation_agent/app/main.py` (180 行) + `evaluation_subgraph.py` (270 行)
