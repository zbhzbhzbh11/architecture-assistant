...
>
> **网关层**，api-gateway，这是系统唯一的对外入口。负责请求校验、缓存检查、LangGraph StateGraph 编排。4 节点串行执行：extract → match → evaluate → trace。
>
> **Agent 层**，4 个业务 Agent。这是系统核心。每个 Agent 有明确的职责边界：
> - requirements-agent 负责特征提取，LLM 独立分析需求语义，词典仅做关键词溯源
> - matching-agent 负责架构匹配，规则评分 + 图谱融合 + 组合推荐
> - evaluation-agent 负责最终评估，LLM 投票 + 摘要 + ADR 生成
> - refactoring-agent 负责重构建议，LLM 优先分析、LLM 可选润色
>
...
### PPT 第 7 页：LangGraph StateGraph + Subgraph 展开
...
graph LR
    ...
    subgraph "matching-agent Subgraph (3 子节点)"
        direction LR
        MS((START)) --> rule_score --> graph_blend --> combo_rank --> ME((END))
    end

    subgraph "evaluation-agent Subgraph (4 子节点)"
        direction LR
        ES((START)) --> sort
        sort -->|Send 并行| vote
...
>
> **Step 1 特征提取**：Gateway 调用 requirements-agent。Agent 由 LLM 优先独立分析需求语义，10-shot Few-shot 输出全维度判断。词典随后为 LLM 判断补充关键词证据。LLM 不可用时回退纯规则提取。
>
> **Step 2 架构匹配**：Gateway 调用 matching-agent。Agent 内部使用 **LangGraph Subgraph** 驱动 3 子节点——rule_score 从 knowledge-base 拉取 10 种风格和学习权重并逐风格评分，graph_blend 获取 Neo4j 图谱证据进行融合并选出 Top 3，combo_rank 生成组合推荐。Neo4j 不可用时图谱得分为 0，不阻断。
>
> **Step 3 评估决策**：Gateway 调用 evaluation-agent。Agent **并行**调用 LLM 做两件事——投票（从给定候选列表选出最佳风格）和摘要（生成中文推荐报告）。并行调用通过 LangGraph Send() API 声明式实现——图结构直接声明 vote 和 summary 两个节点可同时执行，引擎自动管理并发调度。
>
...
>
> 现在看编排引擎（PPT 第 7 页）。系统使用 LangGraph StateGraph 编排——4 节点顺序执行：extract → match → evaluate → trace。每个节点是一个独立的异步 HTTP 调用，节点间通过 shallow merge 自动传递状态。
>
> 值得关注的是两个 Subgraph 的展开结构：
> - **matching-agent Subgraph**：3 子节点 rule_score → graph_blend → combo_rank，纯函数评分 + 图谱融合 + 组合排序
> - **evaluation-agent Subgraph**：4 子节点，其中 vote 和 summary 通过 LangGraph Send() API 声明式并行执行——图结构直接声明两个节点可同时运行，引擎自动管理并发和结果合并
>
> 为什么是 **Workflow-based Multi-Agent** 而不是完全自主 Agent？
>
> 完全自主 Agent 意味着每个 Agent 自己决定下一步调谁、传什么参数——这在课程项目中引入了不必要的复杂性。我们的场景是一个确定性的 Pipeline：一定是先提取特征，再匹配架构，最后评估——不存在动态路由需求。所以 **StateGraph + 顺序节点 + Subgraph 嵌套**是最合适的方案，简单、可调试、可预测。

### 答辩老师可能追问

**Q: 为什么用 Subgraph 而不是平铺所有节点？**
> A: Subgraph 实现了关注点分离——matching-agent 内部的 3 个子节点（评分、融合、排序）和 evaluation-agent 内部的 4 个子节点（排序、投票、摘要、合并）在各自的子图中独立管理，父图只需要知道"调用 matching-agent"和"调用 evaluation-agent"，不需要关心内部实现细节。这符合软件工程的封装原则。
...
>
> **Layer 1 规则引擎**，始终运行，是系统的"安全网"。评分公式很简单：`标签基础分 + 学习权重分 + 特定规则加成`。标签匹配 +2 分一项，学习权重 ≥2 次确认的关联 +1 分，还有 6 条硬编码的领域规则——比如 Event-Driven 遇上高并发额外 +1 分。
>
...
│     每层独立可降级，不是单一 LLM 黑盒               │
│                                                  │
│  2. 微服务 + LangGraph Subgraph 编排                │
│     8 容器 / 4 Agent / StateGraph + Send() 并行     │
│     职责单一，单文件 ≤ 393 行                      │
│                                                  │
│  3. 混合推理 + 幻觉四道防线                        │
│     规则保证下限，LLM 提升上限                     │
...
>
> **第二，微服务 + LangGraph Subgraph 编排。** 8 个 Docker 容器，4 个业务 Agent，每个 Agent 职责单一、代码量可控。LangGraph StateGraph 编排 4 节点顺序执行，matching-agent 和 evaluation-agent 内部采用 Subgraph 封装——父图只管 Agent 间调度，子图管理 Agent 内部流程。
>
...
| 编排引擎 | LangGraph StateGraph + Subgraph | Airflow / 纯手动 | 状态管理标准化、Subgraph 封装内聚、Send() 声明式并行 | 需安装 langgraph 依赖 |
...
| 10 | 学习权重的算法是否有更优方案？ | 1.0 版简单计数累加——已知局限（无限增长、无衰减、无冷启动加速）。改进方向：① EWMA 衰减 `W = α·W_old + (1-α)·n`；② 对数压缩 `W = log(1+n)` 防权重爆炸；③ Thompson Sampling 自适应阈值取代固定 ≥2；④ 引入反馈时效权重（越新越重）。当前框架支持平滑升级——`score_style()` 只读 `learned_weights` 的值，替换 `_compute_weights_from_neo4j()` 的计算逻辑即可。 | 7:30-9:30 |
...
### LangGraph StateGraph + Subgraph 展开
