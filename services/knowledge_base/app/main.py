...
# ═══════════════════════════════════════════════════════════════
BACKEND = os.getenv("KNOWLEDGE_BACKEND", "auto").strip().lower()
...
def graph_risks(style_name: str) -> Dict[str, Any]:
    """查询指定风格的 HAS_RISK 关系 (用于 evaluation 阶段).

    Neo4j 不可用时从 JSON 中查询 STYLE_RISK_MAP.
    """
    if _prefer_graph():
        result = GraphRepository.graph_risks(style_name)
        if result is not None:
            return result
    return JsonRepository.graph_risks(style_name)
